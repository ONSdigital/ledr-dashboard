...
public class DashboardServiceImpl implements DashboardService {
  ...
  public RecordSummary retrieveBirthDashboardData(final String period) throws CannotRetrieveDashboardData{
    ...
    try {
  	  RecordSummary dashboardData = mapper.readValue(new File(fileLocation.getFileLoactionBirth()+period+".json"), RecordSummary.class);
  ...
  public RecordSummary retrieveDeathDashboardData(final String period) throws CannotRetrieveDashboardData{
    ...
    try {
      RecordSummary dashboardData = mapper.readValue(new File(fileLocation.getFileLoactionDeath()+period+".json"), RecordSummary.class);
  	  return dashboardData;
  	}catch (IOException fileReaderError) {
  	  throw new CannotRetrieveDashboardData("cannot find file" ,fileReaderError);
  	}
 ...
 //@Scheduled(fixedRate=5000)
  ...
  //@Scheduled(fixedRate=5000)
  private void callMonthlySqlStatments(){  
    try{
  ...
  //@Scheduled(fixedRate=5000)
  private void callQuartlySqlStatments() {
  	try{
  ...
  //@Scheduled(fixedRate=5000)
  private void callYearlySqlStatments() {
  	try{
  ...
  private void findBirthsDashboardData(TimePeriod period) throws  CannotFindDataException {
    ...
    List<Date> dates = findPeriodRange(period);
	  ...
	  deleteFile(fileLocation.getFileLoactionBirth()+period+".json");
    ...
    try {
      mapper.writeValue(new File(fileLocation.getFileLoactionBirth()+period+".json"),dd);
    } catch (IOException e) {
  ...
  private void findDeathsDashboardData(TimePeriod period) throws CannotFindDataException{
    ...
    deleteFile(fileLocation.getFileLoactionDeath()+period+".json");
    ...
    try {
      mapper.writeValue(new File(fileLocation.getFileLoactionDeath()+period+".json"),dd);
    } catch (IOException e) {
      throw new CannotFindDataException("error mappering data", e);
    }
  }
